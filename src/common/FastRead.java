...
public class FastRead extends Read{	
	...
	public FastRead(FastRead other){
		...
		this.words = wp;
//		score_mask = other.score_mask;
//		segments = other.segments;
//		right_most_bit = other.right_most_bit;
//		read_width = other.read_width;
	}
	...
	public FastRead(char[] s_) {
		...
		for (int i = 0; i < segments; ++i) {			
			for (int j=0;j< Utils.word_size;j++)
				this_seg[j] = s_[i*Utils.word_size+j];
			wpi = new WordM(this_seg);
			wp.add(wpi);
		}
		int i=0;
		char[] last_seg = new char[s_.length-segments*Utils.word_size];
		for (int j=segments*Utils.word_size;j< s_.length;j++)
			last_seg[i++] = s_[j];
	...
	boolean isSsmaller(FastRead rhs) {
		for (int i = 0; i <= segments; ++i)
			if (wp.elementAt(i).isSmaller(rhs.wp.elementAt(i))) return true;
		return false;
	}
	...
	String tostring_bases() {
		String ss="";
		for (int i = 0; i < segments; ++i)
			ss += wp.elementAt(i).tostring_bases(Utils.all_ones);
	...
	String	tostring_bits() {
		String ss="";
		for (int i = 0; i < segments; ++i)
			ss += wp.elementAt(i).tostring_bits(Utils.all_ones) + "\n";
	...
	public void shift(int idx) {
		for (int i = 0; i < segments; ++i)
			wp.elementAt(i).shift(wp.elementAt(i + 1));
		wp.elementAt(segments).shift(idx, right_most_bit);
	}

	@Override
	public int score(Read otherR){
		if (!otherR.getClass().equals(FastRead.class))
			return -1;//not FastRead Class
		FastRead other = (FastRead) otherR;
		int ss = 0;
		for (int i = 0; i < segments; ++i)
			ss += wp.elementAt(i).score(other.wp.elementAt(i), Utils.all_ones);
	...
	public int score_tc(Read otherR) {
		...
		int ss = 0;
		for (int i = 0; i < segments; ++i)
			ss += wp.elementAt(i).score_tc(other.wp.elementAt(i), Utils.all_ones);
	...
	public int score_ag(Read otherR) {
		...
		int ss = 0;
		for (int i = 0; i < segments; ++i)
			ss += wp.elementAt(i).score_ag(other.wp.elementAt(i), Utils.all_ones);
